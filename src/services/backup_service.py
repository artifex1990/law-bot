...
Поддерживаемые хранилища (BACKUP_STORAGE_TYPE):
  ...
  ftp   — FTP / FTPS сервер
  s3    — S3-совместимое облако (AWS, MinIO, Yandex Object Storage и т.д.)

Расписание и ретеншн настраиваются через .env:
  BACKUP_INTERVAL_HOURS  — интервал между бэкапами (по умолчанию 1 ч)
  BACKUP_RETENTION_DAYS  — срок хранения (по умолчанию 30 дней)
"""
...
class BackupService:
    ...
    async def start(self):
        ...
        logger.info(
            f"Backup scheduler started: every {self.interval_hours}h, "
            f"retain {settings.BACKUP_RETENTION_DAYS}d, "
    ...
    async def create_backup(self) -> str | None:
        """Создать один бэкап. Возвращает имя файла или None при ошибке."""
        try:
    ...
    async def _store(self, local_file: Path):
        if self.storage_type == "local":
            ...
            raise ValueError(f"Unknown BACKUP_STORAGE_TYPE: {self.storage_type}")
    ...
    async def _store_local(self, local_file: Path):
        ...
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, shutil.copy2, str(local_file), str(dest))

    async def _cleanup_local(self):
        cutoff = datetime.now(UTC) - self.retention
        if not self.local_path.exists():
            return
        for f in self.local_path.glob(f"{BACKUP_PREFIX}*{BACKUP_SUFFIX}"):
    ...
    def _ftp_upload(self, local_file: Path):
        with self._ftp_connect() as ftp, open(local_file, "rb") as f:
            cmd = f"STOR {local_file.name}"
    ...
    def _cleanup_ftp(self):
        ...
        with self._ftp_connect() as ftp:
            for name in ftp.nlst():
                if not (
                    name.startswith(BACKUP_PREFIX) and name.endswith(BACKUP_SUFFIX)
                ):
                    continue
    ...
    def _parse_timestamp(filename: str) -> datetime | None:
        """Извлечь дату из имени файла вида
        legal_bot_20260406_120000.sql.gz"""
        try:
            core = filename.replace(BACKUP_PREFIX, "").replace(BACKUP_SUFFIX, "")
            dt = datetime.strptime(core, "%Y%m%d_%H%M%S")  # noqa: DTZ007
